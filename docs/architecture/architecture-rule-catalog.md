...
|---|---|---|---|---|
| `modules/core` must not depend on `modules/generator`, `examples`, test support, ArchUnit, JUnit, or third-party runtime packages. | The runtime library must remain small and native-image friendly. | `modules/core` main code. | None. | `CoreArchitectureTest`. |
...
| Core runtime bindings must resolve to non-null values. | Null dependencies hide configuration mistakes and create late unboxing failures for scalar helpers. | `modules/core` main code. | Applications should bind `Optional<T>` or a domain value for nullable data. | `AppContextTest` and `BinderTest`. |
| Module installation is transactional. | Failed module configuration should not leave partial bindings that depend on exception timing. | `modules/core` main code. | None. | `BinderTest`. |
| Runtime dependency cycles must fail with diagnostics. | Recursive provider lookup should fail with an actionable key path instead of overflowing the stack. | `modules/core` main code. | None. | `AppContextTest`. |
...
| `modules/generator` must use only the JDK and `modules/core`. | The generator should not add external runtime dependencies for projects that only need code generation. | `modules/generator` main code. | None. | `GeneratorArchitectureTest`. |
| Build-time generator scanning may use JDK class loading and reflection. | The scanner must inspect annotations on compiled classes, but this happens before application runtime. | `modules/generator` main code. | Only scanner and CLI generation paths; generated source and `modules/core` remain reflection-free. | `GeneratorArchitectureTest` and generated-source tests. |
| Generated `AppPluginModule` source must not reference generator packages, reflection APIs, service loading, class loaders, or test packages. | Generated code must be runtime-only source that can be compiled into Native Image applications without generator reachability. | Generated source emitted by `generator` tests. | None. | `GeneratedModuleSourceArchitectureTest`. |
| Generated `AppPluginModule` source must use singleton bindings and meet project code-quality expectations. | Generated code should be committed and checked like ordinary source in consumer projects. | Generated source emitted by `generator` tests. | None. | Generated source compile, Javadoc, Checkstyle, Google Java Format, behavior, and source-shape tests. |
...
| Native-targeted code must not use classpath-scanning libraries. | Classpath scanning is runtime discovery and does not fit closed-world static analysis. | `modules/core` main code and generated module source. | `modules/generator` may scan with JDK APIs at build time. | Module architecture tests. |
